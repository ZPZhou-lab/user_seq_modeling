...
# Copyright (c) 2025 AntGroup Ltd. and/or its affiliate
#
...
class Qwen3Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if self.config.use_ft_flash_attn and cu_input_lens is not None:
            assert kwargs.get('output_attentions', None) in [None, False]
            # repeat key and value states
            key_states = repeat_kv(key_states, self.num_key_value_groups)
            value_states = repeat_kv(value_states, self.num_key_value_groups)

            # (batch, head, seq_len, head_dim) -> (batch, seq_len, head, head_dim)
            query_states = query_states.transpose(1, 2)
            key_states = key_states.transpose(1, 2)
            value_states = value_states.transpose(1, 2)

            if attention_mask is not None:
                # (batch, 1, tgt_len, src_len) -> (batch, seq_len)
